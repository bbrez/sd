...
function login() {
  ...
  socket.onopen = (e) => {
    ...
    lastCredentials.value = { name: user.value.name, password: user.value.password };
...
const formCheck = computed(() => {
  ...
  if (lastCredentials.value.password != user.value.password) formState.value = 'clean';

  switch (formState.value) {
    case 'clean': break;
    case 'wrong_credentials': errors.push('wrong_credentials'); break;
      ...
      <div class="column is-half">
        <form class="box">
          ...
          <div class="field">
            ...
            <p v-if="formCheck.includes('wrong_credentials')" class="help is-danger">Credenciais Incorretas</p>
          </div>
          <div class="field">
          ...
          <div class="field is-grouped">
            <div class="control">
              <button class="button is-info" :class="{ 'is-loading': formState == 'sending' }" type="button"
                :disabled="formCheck.length > 0" @click="login()">Entrar</button>
            </div>
            <div class="control">
            ...
            <div class="control">
              <button class="button is-link is-light" type="button"
                @click="$router.push({ name: 'register' })">Registrar</button>
            </div>
          </div>
        </form>
      </div>
